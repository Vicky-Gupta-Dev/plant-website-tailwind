...
function defaultExtractor(context) {
   ...
   */ return (content) => {
    ...
    for (let pattern of patterns) {
      var _content_match
      for (let result of (_content_match = content.match(pattern)) !== null &&
      _content_match !== void 0
        ? _content_match
        : []) {
...
function* buildRegExps(context) {
  ...
  let prefix =
    context.tailwindConfig.prefix !== ''
      ? _regex.optional(_regex.pattern([/-?/, _regex.escape(context.tailwindConfig.prefix)]))
      : ''
  ...
  let variantPatterns = [
    ...
    _regex.any([
      ...
      _regex.pattern([/([^\s"'`\[\\]+-)?\[[^\s"'`]+\]\/[\w_-]+/, separator]),
      _regex.pattern([/([^\s"'`\[\\]+-)?\[[^\s"'`]+\]/, separator]),
      _regex.pattern([/[^\s"'`\[\\]+/, separator]),
    ]),
    // With quotes allowed
    _regex.any([
      // With variant modifier (e.g.: group-[..]/modifier)
