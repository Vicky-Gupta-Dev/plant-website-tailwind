...
Command.prototype._optionEx = function (
  config,
  flags,
  description,
  fn,
  ...
  if (
    ...
    if (option.negate) {
      ...
      defaultValue = self.optionFor(positiveLongFlag)
        ? self._getOptionValue(name)
        : true;
    }
  ...
  this.on("option:" + oname, function (val) {
    ...
    if (val !== null && fn) {
      val = fn(
        val,
        self._getOptionValue(name) === undefined
          ? defaultValue
          : self._getOptionValue(name),
      );
    }
    ...
    if (
      typeof self._getOptionValue(name) === "boolean" ||
      typeof self._getOptionValue(name) === "undefined"
    ) {
      // if no value, negate false, and we have a default, then use it!
      if (val == null) {
        self._setOptionValue(
          name,
...
Command.prototype.requiredOption = function (
  flags,
  description,
  fn,
  defaultValue,
...
Command.prototype.storeOptionsAsProperties = function (value) {
  ...
  if (this.options.length) {
    ...
    console.error(
      "Commander usage error: call storeOptionsAsProperties before adding options",
    );
  }
  return this;
};
...
Command.prototype.parse = function (argv) {
  ...
  if (this._execs.has(name)) {
    return this.executeSubCommand(
      argv,
      args,
      parsed.unknown,
      subCommand ? subCommand._executableFile : undefined,
    );
  }
  ...
  return this.options
    .map(function (option) {
      ...
      return (
        ...
        "  " +
        optionalWrap(fullDesc, descriptionWidth, width + 2)
      );
    })
    .concat([
      pad(this._helpFlags, width) +
        "  " +
        optionalWrap(this._helpDescription, descriptionWidth, width + 2),
    ])
    .join("\n");
};
