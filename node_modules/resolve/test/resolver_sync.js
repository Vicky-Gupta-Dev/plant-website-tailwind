...
test("foo", function (t) {
  ...
  t.equal(resolve.sync("./foo", { basedir: dir }), path.join(dir, "foo.js"), "./foo");
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./foo", { basedir: dir }), require.resolve("./foo", { paths: [dir] }), "./foo: resolve.sync === require.resolve");
  }

  t.equal(resolve.sync("./foo.js", { basedir: dir }), path.join(dir, "foo.js"), "./foo.js");
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./foo.js", { basedir: dir }), require.resolve("./foo.js", { paths: [dir] }), "./foo.js: resolve.sync === require.resolve");
  }

  t.equal(resolve.sync("./foo.js", { basedir: dir, filename: path.join(dir, "bar.js") }), path.join(dir, "foo.js"));
...
test("bar", function (t) {
  ...
  t.equal(resolve.sync("foo", { basedir: basedir }), path.join(dir, "bar/node_modules/foo/index.js"), "foo in bar");
  if (!requireResolveDefaultPathsBroken && requireResolveSupportsPaths) {
    t.equal(resolve.sync("foo", { basedir: basedir }), require.resolve("foo", { paths: [basedir] }), "foo in bar: resolve.sync === require.resolve");
  }
...
test("baz", function (t) {
  ...
  t.equal(resolve.sync("./baz", { basedir: dir }), path.join(dir, "baz/quux.js"), "./baz");
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./baz", { basedir: dir }), require.resolve("./baz", { paths: [dir] }), "./baz: resolve.sync === require.resolve");
  }
...
test("biz", function (t) {
  ...
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./grux", { basedir: dir }), require.resolve("./grux", { paths: [dir] }), "./grux: resolve.sync === require.resolve");
  }

  var tivDir = path.join(dir, "grux");
  t.equal(resolve.sync("tiv", { basedir: tivDir }), path.join(dir, "tiv/index.js"));
  if (!requireResolveDefaultPathsBroken && requireResolveSupportsPaths) {
    t.equal(resolve.sync("tiv", { basedir: tivDir }), require.resolve("tiv", { paths: [tivDir] }), "tiv: resolve.sync === require.resolve");
  }

  var gruxDir = path.join(dir, "tiv");
  t.equal(resolve.sync("grux", { basedir: gruxDir }), path.join(dir, "grux/index.js"));
  if (!requireResolveDefaultPathsBroken && requireResolveSupportsPaths) {
    t.equal(resolve.sync("grux", { basedir: gruxDir }), require.resolve("grux", { paths: [gruxDir] }), "grux: resolve.sync === require.resolve");
  }
...
test("cup", function (t) {
  ...
  t.equal(resolve.sync("./cup.coffee", { basedir: dir }), path.join(dir, "cup.coffee"), "./cup.coffee");
  ...
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./cup.coffee", { basedir: dir, extensions: [".js", ".coffee"] }), require.resolve("./cup.coffee", { paths: [dir] }), "./cup.coffee: resolve.sync === require.resolve");
  }
...
test("mug", function (t) {
  ...
  t.equal(resolve.sync("./mug", { basedir: dir }), path.join(dir, "mug.js"), "./mug -> ./mug.js");
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./mug", { basedir: dir }), require.resolve("./mug", { paths: [dir] }), "./mug: resolve.sync === require.resolve");
  }
...
test("path iterator", function (t) {
  ...
  t.equal(resolve.sync("baz", { packageIterator: exactIterator }), path.join(resolverDir, "baz/quux.js"));
...
test("incorrect main", function (t) {
  ...
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./incorrect_main", { basedir: resolverDir }), require.resolve("./incorrect_main", { paths: [resolverDir] }), "./incorrect_main: resolve.sync === require.resolve");
  }
...
test("main: false", function (t) {
  ...
  t.equal(resolve.sync("./false_main", { basedir: basedir }), path.join(dir, "index.js"), '`"main": false`: resolves to `index.js`');
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./false_main", { basedir: basedir }), require.resolve("./false_main", { paths: [basedir] }), '`"main": false`: resolve.sync === require.resolve');
  }
...
test('#52 - incorrectly resolves module-paths like "./someFolder/" when there is a file of the same name', function (t) {
  ...
  t.equal(resolve.sync("./foo", { basedir: basedir }), path.join(dir, "same_names/foo.js"));
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./foo", { basedir: basedir }), require.resolve("./foo", { paths: [basedir] }), "./foo: resolve.sync === require.resolve");
  }

  t.equal(resolve.sync("./foo/", { basedir: basedir }), path.join(dir, "same_names/foo/index.js"));
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./foo/", { basedir: basedir }), require.resolve("./foo/", { paths: [basedir] }), "./foo/: resolve.sync === require.resolve");
  }
...
test('#211 - incorrectly resolves module-paths like "." when from inside a folder with a sibling file of the same name', function (t) {
  ...
  t.equal(resolve.sync("./", { basedir: basedir }), path.join(dir, "same_names/foo/index.js"), "./");
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./", { basedir: basedir }), require.resolve("./", { paths: [basedir] }), "./: resolve.sync === require.resolve");
  }

  t.equal(resolve.sync(".", { basedir: basedir }), path.join(dir, "same_names/foo/index.js"), ".");
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync(".", { basedir: basedir }), require.resolve(".", { paths: [basedir] }), ".: resolve.sync === require.resolve", { todo: true });
  }
...
test("absolute paths", function (t) {
  ...
  t.equal(resolve.sync(__filename, { basedir: process.cwd() }), __filename, "absolute path to this file with a basedir resolves");
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync(__filename, { basedir: process.cwd() }), require.resolve(__filename, { paths: [process.cwd()] }), "absolute path to this file + basedir: resolve.sync === require.resolve");
  }

  t.equal(resolve.sync(extensionless, { basedir: process.cwd() }), __filename, "extensionless absolute path to this file with a basedir resolves");
  if (requireResolveSupportsPaths) {
    t.equal(
...
test("malformed package.json", { skip: !fs.existsSync(malformedDir) }, function (t) {
  ...
  t.equal(resolve.sync("./index.js", { basedir: basedir }), expected, "malformed package.json is silently ignored");
  if (requireResolveSupportsPaths) {
    t.equal(resolve.sync("./index.js", { basedir: basedir }), require.resolve("./index.js", { paths: [basedir] }), "malformed package.json: resolve.sync === require.resolve");
  }
