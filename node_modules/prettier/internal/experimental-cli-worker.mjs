  ...
  return function retryified(options2) {
    ...
    return function attempt3(...args) {
      return fn.apply(void 0, args).catch((error) => {
        ...
        if (delay > 0) {
          const delayPromise = new Promise((resolve3) =>
...
var Temp = {
  ...
  truncate: (filePath) => {
    ...
    const truncable =
      /^(\.?)(.*?)((?:\.[^.]+)?(?:\.tmp-\d{10}[a-f0-9]{6})?)$/.exec(basename);
...
async function writeFileAsync(filePath, data, options = DEFAULT_WRITE_OPTIONS) {
  ...
  try {
    ...
    fd = await dist_default.retry.open(retryOptions)(
      tempPath,
      "w",
    ...
    if (isString(data)) {
      await dist_default.retry.write(retryOptions)(
        fd,
        data,
        0,
    ...
    if (
      ...
      await dist_default.attempt.chown(
        tempPath,
        options.chown.uid,
        options.chown.gid,
      );
    ...
    try {
      ...
      await dist_default.retry.rename(retryOptions)(
        tempPath,
  ...
  if (node.children?.length) {
    ...
    if (children?.length) {
      ...
      const needsWrapperGroup =
        children.length > 1 ||
        (isNodePartial && (!source.length || isSomeChildNonPartial));
  ...
  return (rule) => {
    ...
    return (state) => {
      ...
      if (isPotentiallyCached) {
        const cacheStore =
          cache2.store || (cache2.store = /* @__PURE__ */ new Map());
