        ...
        case "object":
          if ("env" in vendor.pr) {
            if ("any" in vendor.pr) {
              ...
              return (
                vendor.pr.env in env3 && env3[vendor.pr.env] !== vendor.pr.ne
              );
            }
...
var createMethodShim =
  ...
  (flags, object2, ...arguments_) => {
    ...
    const implementation =
      getImplementation.call(object2) ?? object2[methodName];
...
var preserveConsecutiveUppercase = (input, toLowerCase) =>
  input.replace(LEADING_CAPITAL, (match) => toLowerCase(match));
var processWithCasePreservation = (
  input,
  toLowerCase,
  preserveConsecutiveUppercase2,
) => {
  ...
  for (let index = 0; index < characters.length; index++) {
    ...
    const nextCharIsUpperCase =
      index + 1 < characters.length && UPPERCASE.test(characters[index + 1]);
...
var normalizeToPosix =
  path.sep === "\\"
    ...
    : (filepath) => filepath;
var { omit } = sharedWithCli2.utilities;
...
function parseArgv(rawArguments, detailedOptions, logger, keys2) {
  ...
  const normalized = normalize_cli_options_default(argv2, detailedOptions, {
    logger,
  });
  return {
    ...Object.fromEntries(
...
async function logFileInfoOrDie(context) {
  ...
  const result = await format(
    (0, import_fast_json_stable_stringify.default)(fileInfo),
    { parser: "json" },
  );
...
function findUpSync(
  name,
  { cwd: cwd3 = process2.cwd(), type = "file", stopAt } = {},
) {
...
function findCacheDirectory(options = {}) {
  ...
  return useDirectory(
    path8.join(directory, "node_modules", ".cache", options.name),
    options,
  );
}
    ...
    if (this.isRelativePath(filePath)) {
      ...
      if (this._restrictAccessToCwd) {
        ...
        const isWithinCwd =
          ...
          normalizedResolved.startsWith(normalizedCwd + path11.sep);
        if (!isWithinCwd) {
          throw new Error(
...
async function formatFiles(context) {
  ...
  const isTTY =
    mockable_default.isStreamTTY(process.stdout) && !mockable_default.isCI();
...
async function main(context) {
  ...
  const useStdin =
    !hasFilePatterns &&
    (!mockable_default.isStreamTTY(process.stdin) || context.argv.filepath);
  if (useStdin) {
    if (context.argv.cache) {
