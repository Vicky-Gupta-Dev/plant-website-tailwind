    ...
    createMethodShim =
      ...
      (flags, object, ...arguments_) => {
        ...
        const implementation =
          getImplementation.call(object) ?? object[methodName];
...
var init_dist5 = __esm({
  "node_modules/tiny-parse-argv/dist/index.js"() {
    ...
    getAliasedSet = (aliases, values = []) => {
      ...
      const valuesAliased = /* @__PURE__ */ new Set([
        ...values,
        ...valuesAliases,
      ]);
    ...
    parseEqualsSeparator = (argv) => {
      const re = /^(--?[^=][^=]*?)=([^]*)$/;
      const shieldValue = (value) =>
        value.startsWith("-") ? `"${value}"` : value;
      return parseWithRegExp(argv, re, (_, key2, value) => [
        key2,
        shieldValue(value),
      ]);
    };
    ...
    parseProto = (argv) => {
      ...
      return argv.filter(
        (arg, index) => !re.test(arg) && !re.test(argv[index - 1]),
      );
    };
    parseOption = (arg) => {
      const optionRe = /^(--?)([^]+)$/;
    ...
    parseArgv = (argv, options = {}) => {
      ...
      const incompatibles = getAliasedIncompatibles(
        aliases,
      ...
      if (onMissing) {
        const missings = required.filter(
          (key2) => !(key2 in parsedWithDefaults),
        );
        if (missings.length) {
          onMissing(missings);
        }
      ...
      if (onInvalid) {
        const invalids = found.filter(
          (key2) => parsedWithDefaults[key2] === void 0,
        );
        if (invalids.length) {
          onInvalid(invalids);
        }
  ...
  "node_modules/tiny-updater/dist/utils.js"() {
    ...
    Utils = {
      ...
      notify: (name, version, latest) => {
        if (!globalThis.process?.stdout?.isTTY) return;
        const log = () =>
          console.log(`
  ...
  "node_modules/fast-string-truncated-width/dist/index.js"() {
    ...
    getStringTruncatedWidth = (
      ...
      const ELLIPSIS_WIDTH2 =
        ...
        (ELLIPSIS2
          ? getStringTruncatedWidth(ELLIPSIS2, NO_TRUNCATION, widthOptions)
              .width
          : 0);
      const ANSI_WIDTH = 0;
  ...
  "node_modules/tiny-bin/dist/objects/logger.js"() {
    ...
    Logger = class extends addon_default {
      ...
      print(message = "") {
        ...
        this.handler(
          colorize(
            `${this.indentation.repeat(this.indentationLevel)}${message}`,
          ),
        );
      }
      table(rows, mode = "line") {
        const rowsLengths = rows.map((row) =>
          row.map((cell) => dist_default8(cell)),
        );
  ...
  "node_modules/tiny-bin/dist/objects/collection.js"() {
    ...
    Collection = class extends addon_default {
      ...
      getByIdOrFail(id) {
        ...
        this.bin.fail(
          `Not found "${id}"${closest ? `. Did you mean "${closest}"?` : ""}`,
        );
      }
      getByIds(ids) {
        for (const id of ids) {
      ...
      print(mode) {
        ...
        commandsBySection.forEach((commands2, section) => {
          ...
          const table = commands2.map((command) => {
            const withDeprecated = command.deprecated
              ? dist_default4.dim
              : identity;
            return [
              [
    ...
    Options = class extends collection_default {
      ...
      print(mode) {
        ...
        const optionsBySection = pushBack(
          groupBy(optionsVisible, (option) =>
            withoutOther(option.section.toLowerCase()),
          ),
          "",
        );
        optionsBySection.forEach((options2, section) => {
          ...
          const table = options2.map((option) => {
            const withDeprecated = option.deprecated
              ? dist_default4.dim
              : identity;
            return [
              [
  ...
  "node_modules/tiny-bin/dist/objects/command.js"() {
    ...
    Command = class extends addon_default {
      ...
      async run(options, argv) {
        if (!this.handler) {
          this.bin.fail(
            `Command handler not defined for command: "${this.name}"`,
          );
        } else {
...
var init_command_default = __esm({
  "node_modules/tiny-bin/dist/objects/command_default.js"() {
    ...
    CommandDefault = class extends command_default {
      ...
      /* API */
      async run(options, argv) {
        const name = this.bin.commands.getById(options._[0])
          ? options._[0]
          : this.name;
        if (options["help"] || name === "help") {
          ...
          const minArgs = command.arguments
            .getAll()
            .filter((arg) => arg.required).length;
          const maxArgs = sum(
            command.arguments
              .getAll()
              .map((arg) => (arg.variadic ? Infinity : 1)),
          );
          ...
          options2.forEach((option) => {
            ...
            if (option.incompatible) {
              const incompatible =
                (_a = parseArgvOptions.incompatible)[
                  (_b = option.data.alls[0])
                ] || (_a[_b] = []);
            ...
            if (option.validate) {
              parseArgvOptions.validators[option.data.alls[0]] =
                option.validate;
            }
  ...
  "node_modules/tiny-bin/dist/objects/chainable_command.js"() {
    ...
    ChainableCommand = class extends addon_default {
      ...
      option(name, description, options = {}) {
        const option = new option_default(this.bin, {
          name,
          description,
          ...options,
        });
      ...
      argument(name, description, options = {}) {
        const argument = new argument_default(this.bin, {
          name,
          description,
          ...options,
        });
...
var init_chainable_bin_global = __esm({
  "node_modules/tiny-bin/dist/objects/chainable_bin_global.js"() {
    ...
    ChainableBin = class extends addon_default {
      ...
      command(name, description, options = {}) {
        const command = new command_default(this.bin, {
          name,
          description,
          ...options,
        });
...
var ERR_UNKNOWN_FILE_EXTENSION,
  hasOwnProperty2,
  extensionFormatMap,
  protocolHandlers;
...
function packageExportsResolve(
  ...
  while (++i < keys.length) {
    ...
    if (
      patternIndex !== -1 &&
      packageSubpath.startsWith(key2.slice(0, patternIndex))
    ) {
  ...
  "node_modules/tiny-readdir/dist/index.js"() {
    ...
    readdir = (rootPath, options) => {
      ...
      const ignores = castArray3(ignore).map((ignore2) =>
        isFunction(ignore2)
          ? ignore2
          : (targetPath) => ignore2.test(targetPath),
      );
      const isIgnored = (targetPath) =>
        ignores.some((ignore2) => ignore2(targetPath));
      ...
      const handleFile = (dirmap, subPath, name) => {
        ...
        files.push(subPath);
        filesNames.add(name);
        filesNamesToPaths.propertyIsEnumerable(name) ||
          (filesNamesToPaths[name] = []);
  ...
  "node_modules/graphmatch/dist/utils.js"() {
    ...
    getNodeSourceWithCache = (node, partial, cache3) => {
      ...
      if (node.children?.length) {
        const children = uniq2(
          node.children
            .map((node2) => getNodeSourceWithCache(node2, partial, cache3))
            .filter(Boolean),
        );
        if (children?.length) {
          const isSomeChildNonPartial = node.children.some(
            (child) => !child.regex || !(child.partial ?? partial),
          );
          const needsWrapperGroup =
            children.length > 1 ||
            (isNodePartial && (!source2.length || isSomeChildNonPartial));
  ...
  "node_modules/grammex/dist/index.js"() {
      ...
      return (state) => {
        ...
        if (matched) {
          ...
          if (!isUndefined3(handler) && !state.options.silent) {
            const output = isFunction2(handler)
              ? handler(
                  input.slice(indexStart, indexEnd),
                  input,
                  `${indexStart}`,
                )
              : handler;
    ...
    backtrackable = (rule, enabled = true, force = false) => {
      ...
      return (state) => {
        ...
        const matched = erule(state);
        if (!matched && !force) {
          state.indexBacktrackMax = Math.max(
            state.indexBacktrackMax,
            state.index,
          );
        }
        if (!matched || force) {
    ...
    memoizable = /* @__PURE__ */ (() => {
      ...
      return (rule) => {
        ...
        return (state) => {
          ...
          const indexStart = state.index;
          const cache3 =
            (_a = state.cache)[ruleId] ||
            (_a[ruleId] = { indexMax: -1, queue: [] });
...
var init_grammar2 = __esm({
  "node_modules/zeptomatch/dist/parse/grammar.js"() {
    ...
    NegationOdd = match(/^(?:!!)*!(.*)$/, (_, glob) =>
      regex2(`(?!^${dist_default19.compile(glob).source}$).*?`),
    );
    NegationEven = match(/^(!!)+/);
    Negation = or([NegationOdd, NegationEven]);
    StarStarBetween = match(/\/(\*\*\/)+/, () =>
      alternation([sequence([slash(), regex2(".+?"), slash()]), slash()]),
    );
    StarStarStart = match(/^(\*\*\/)+/, () =>
      alternation([regex2("^"), sequence([regex2(".*?"), slash()])]),
    );
    StarStarEnd = match(/\/(\*\*)$/, () =>
      alternation([sequence([slash(), regex2(".*?")]), regex2("$")]),
    );
    StarStarNone = match(/\*\*/, () => regex2(".*?"));
    StarStar = or([StarStarBetween, StarStarStart, StarStarEnd, StarStarNone]);
    StarDouble = match(/\*\/(?!\*\*\/|\*$)/, () =>
      sequence([regex2("[^\\\\/]*?"), slash()]),
    );
    ...
    Class = and(
      [ClassOpen, optional(ClassNegation), star(ClassValue), ClassClose],
      (_) => regex2(_.join("")),
    );
    RangeOpen = match("{", "(?:");
    RangeClose = match("}", ")");
    ...
    RangeAlphaUpper = match(/([A-Z]+)\.\.([A-Z]+)/, (_, $1, $2) =>
      makeRangeAlpha($1.toLowerCase(), $2.toLowerCase())
        .join("|")
        .toUpperCase(),
    );
    ...
    Braces = and(
      [
        BracesOpen,
        optional(and([BracesValue, star(and([BracesComma, BracesValue]))])),
        BracesClose,
      ],
      alternation,
    );
    ...
    explodeStart = (glob) => {
      ...
      while (index < length) {
        ...
        const values = multiple
          ? multiple
              .split(",")
              .map((value) => `${slash2}${prefix}${value}${suffix}`)
          : [`${slash2}${prefix}${suffix}`];
        statics = statics.length
          ? statics.flatMap((prefix2) =>
              values.map((value) => `${prefix2}/${value}`),
            )
          : values;
...
var init_explode_end = __esm({
  "node_modules/zeptomatch-explode/dist/explode_end.js"() {
    ...
    explodeEnd = (glob) => {
      ...
      if (match2) {
        const [_, slash2, starStart, prefix, multiple, suffix, starEnd] =
          match2;
...
var init_utils14 = __esm({
  "node_modules/tiny-readdir-glob/dist/utils.js"() {
    ...
    globCompile = (glob) => {
      ...
      const { flexibleStart, flexibleEnd, statics, dynamic } =
        explode_end_default(glob);
    ...
    globsCompile = (globs) => {
      ...
      return (rootPath, targetPath) =>
        fns.some((fn) => fn(rootPath, targetPath));
    };
    ...
    ignoreCompile = (rootPath, ignore) => {
      ...
      return castArray4(ignore).map((ignore2) => {
        if (!isString3(ignore2)) return ignore2;
    ...
    readdirGlob = async (glob, options) => {
      ...
      for (const [folders, foldersGlobs] of globsExplode(globsPositive)) {
        ...
        for (const folder of folders) {
          ...
          bucketDirectoriesFoundNamesToPaths.push(
            result.directoriesNamesToPaths,
          );
      ...
      const filesFoundNamesToPaths = uniqMergeConcat(
        bucketFilesFoundNamesToPaths,
      );
      const symlinksFoundNamesToPaths = uniqMergeConcat(
        bucketSymlinksFoundNamesToPaths,
      );
      return {
        directories,
        files,
...
async function getFolderChildrenPaths(folderPath) {
  ...
  const childrenPaths = dirents.map((dirent) =>
    fastJoinedPath(folderPath, dirent.name),
  );
  return childrenPaths;
}
async function getFoldersChildrenPaths(foldersPaths) {
  const childrensPaths = await Promise.all(
    foldersPaths.map(getFolderChildrenPaths),
  );
...
async function getModule(modulePath) {
  ...
  const module =
    moduleExports.default || moduleExports.exports || moduleExports;
  return module;
}
...
async function getTargetsPaths(rootPath, globs, withNodeModules) {
  ...
  const directoriesResults = await Promise.all(
    targetDirectories.map((targetPath) =>
      getDirectoryPaths(targetPath, withNodeModules),
    ),
  );
  const directoriesResultsFiles = directoriesResults.map(
    (result) => result.files,
  );
  ...
  const filesNames = [
    ...without2(targetFilesNames, foundFilesNames),
    ...foundFilesNames,
  ];
  ...
  const directoryFoldersFoundPaths = directoriesResults.flatMap(
    (result) => result.directories,
  );
  const foldersFoundPaths = [
    rootPath,
    ...globFoldersFoundPaths,
    ...directoryFoldersFoundPaths,
  ];
  return [
    filesPaths,
    filesNames,
...
async function normalizeOptions(options, targets) {
  ...
  const targetsStatic =
    "--" in options && Array.isArray(options["--"])
      ? options["--"].filter(isString4).map(dist_default23)
      : [];
  const globs = [...targetsGlobs, ...targetsStatic];
  const stdin = await getStdin();
  if (!isString4(stdin) && !globs.length)
  ...
  if (check && list)
    exit_default(
      'The "--check" and "--list-different" flags cannot be used together',
    );
  if (check && write)
  ...
  const parallelWorkers =
    ("parallelWorkers" in options &&
      Math.round(Number(options.parallelWorkers))) ||
    0;
  const stdinFilepath =
    "stdinFilepath" in options && isString4(options.stdinFilepath)
...
function normalizePrettierOptions(options, folderPath) {
  ...
  if ("overrides" in options && isArray3(options.overrides)) {
    ...
    for (let i = 0, l = overridesRaw.length; i < l; i++) {
      ...
      if (
        !isString4(overrideRaw.files) &&
        (!isArray3(overrideRaw.files) || !overrideRaw.files.every(isString4))
      )
        continue;
      ...
      const filesNegative =
        ...
        (isString4(overrideRaw.filesNegative) ||
          (isArray3(overrideRaw.filesNegative) &&
            overrideRaw.filesNegative.every(isString4)))
  ...
  "node_modules/@prettier/cli/dist/cache.js"() {
    ...
    Cache = class {
      constructor(version, rootPath, options, logger) {
        ...
        this.storePath =
          options.cacheLocation ||
          path11.join(getCachePath(rootPath), `${sha1hex(rootPath)}.json`);
      ...
      get(filePath) {
        ...
        try {
          ...
          if (!isString4(hash) || !isBoolean2(formatted)) return { save };
      ...
      set(filePath, fileRelativePath, fileFormatted, fileContentExpected) {
        ...
        try {
          const version =
            (_a = this.store)[(_b = this.version)] || (_a[_b] = {});
...
var init_utils16 = __esm({
  "node_modules/ini-simple-parser/dist/utils.js"() {
    inferBoolean = (value) => {
      if (!isString5(value) || !value.length) return value;
    ...
    resolve3 = (configs, filePath) => {
      ...
      for (let i = 0, l = configs.length; i < l; i++) {
        ...
        for (const override in overrides) {
          const glob = `**/${override}`;
          if (override !== "*" && !dist_default19(glob, filePath)) continue;
    ...
    getEditorConfigsMap = async (foldersPaths, filesNames) => {
      const configs = await Promise.all(
        foldersPaths.map((folderPath) =>
          getEditorConfig(folderPath, filesNames),
        ),
      );
...
var init_grammar3 = __esm({
  "node_modules/fast-ignore/dist/glob/grammar.js"() {
    ...
    Class2 = and([
      ClassOpen2,
      optional(ClassNegation2),
      star(ClassValue2),
      ClassClose2,
    ]);
    Grammar3 = star(
      or([Star2, Question2, Class2, Escaped3, Escape2, Passthrough3]),
    );
...
var init_parse3 = __esm({
  "node_modules/fast-ignore/dist/glob/parse.js"() {
    ...
    _parse2 = (glob, caseSensitive) => {
      const source2 = parse(glob, grammar_default3, {
        memoization: false,
      }).join("");
    ...
    matcher = (glob, caseSensitive) => {
      ...
      if (STATIC_RE.test(glob)) {
        if (caseSensitive) {
          ...
          return (segment) =>
            segment.length === globLength &&
            segment.toLowerCase() === globLowerCase;
        }
      }
      ...
      for (let ti = 0, tl = tiers.length; ti < tl; ti++) {
        ...
        for (let gi = 0, gl = globs.length; gi < gl; gi++) {
          ...
          for (let si = 0, sl = segments.length; si < sl; si++) {
            ...
            const nodeExisting = parent.children.find(
              (node2) => node2.id === id,
            );
            if (nodeExisting) {
              if (
                (tier === nodeExisting.tier &&
                  strength >= nodeExisting.strength) ||
                (tier > nodeExisting.tier &&
  ...
  "node_modules/fast-ignore/dist/ignore/parse.js"() {
    ...
    parse4 = (ignore) => {
      ...
      for (let i = 0, l = lines.length; i < l; i++) {
        ...
        content = content.replace(/((?:\\\s)*)\s*$/, ($0, $1) =>
  ...
  "node_modules/@prettier/cli/dist/config_ignore.js"() {
    ...
    getIgnoresContentMap = async (foldersPaths, filesNames) => {
      const contents = await Promise.all(
        foldersPaths.map((folderPath) =>
          getIgnoresContent(folderPath, filesNames),
        ),
      );
    ...
    getIgnoresUp = dist_default13(async (folderPath, filesNames) => {
      ...
      const ignoreUp =
        folderPath !== folderPathUp
          ? await getIgnoresUp(folderPathUp, filesNames)
          : void 0;
...
function skipVoid(str2, ptr, banNewLines, banComments) {
  ...
  while (
    ...
    ptr++;
  return banComments || c2 !== "#"
    ? ptr
    : skipVoid(str2, skipComment(str2, ptr), banNewLines);
}
...
function extractValue(str2, ptr, end, depth, integersAsBigInt) {
  ...
  if (c2 === "[" || c2 === "{") {
    let [value, endPtr2] =
      c2 === "["
        ? parseArray(str2, ptr, depth, integersAsBigInt)
        : parseInlineTable(str2, ptr, depth, integersAsBigInt);
  ...
  return [parseValue2(slice[0], str2, ptr, integersAsBigInt), endPtr];
}
...
function getLine(buffer2, lineStart, lineEnd, position, maxLineLength) {
  ...
  return {
    str:
      head + buffer2.slice(lineStart, lineEnd).replace(/\t/g, "\u2192") + tail,
...
function makeSnippet(mark, options) {
  ...
  result +=
    ...
    "\n";
  result +=
    common.repeat("-", options.indent + lineNoLength + 3 + line2.pos) + "^\n";
...
function isHexCode(c2) {
  return (
    (48 <= c2 && c2 <= 57) || (65 <= c2 && c2 <= 70) || (97 <= c2 && c2 <= 102)
  );
}
  ...
  "node_modules/@prettier/cli/dist/config_prettier.js"() {
    ...
    Loaders = {
      auto: (filePath) => {
        ...
        const loader2 =
          File2Loader[basename] || Ext2Loader[ext] || File2Loader["default"];
      ...
      json5: async (filePath) => {
        const fileContent = fs11.readFileSync(filePath, "utf8");
        const JSON5 = (
          await Promise.resolve().then(() => (init_dist32(), dist_exports2))
        ).default;
      ...
      toml: async (filePath) => {
        const fileContent = fs11.readFileSync(filePath, "utf8");
        const { parse: parse8 } = await Promise.resolve().then(
          () => (init_dist33(), dist_exports3),
        );
        return parse8(fileContent);
      },
      yaml: async (filePath) => {
        const yaml = (
          await Promise.resolve().then(() => (init_js_yaml(), js_yaml_exports))
        ).default;
        const fileContent = fs11.readFileSync(filePath, "utf8");
    ...
    getPrettierConfigsMap = async (foldersPaths, filesNames) => {
      const configs = await Promise.all(
        foldersPaths.map((folderPath) =>
          getPrettierConfigs(folderPath, filesNames),
        ),
      );
      const map2 = zipObjectUnless(foldersPaths, configs, isUndefined4);
      return map2;
      ...
      for (let ci = 0, cl = configs.length; ci < cl; ci++) {
        ...
        if (overrides) {
          for (let oi = 0, ol = overrides.length; oi < ol; oi++) {
            ...
            const filePathRelative = fastRelativeChildPath(
              override.folder,
              filePath,
            );
...
var init_dist39 = __esm({
  "node_modules/tiny-truncate/dist/index.js"() {
    ...
    truncate2 = (str2, options) => {
      const width =
        (globalThis.process?.stdout?.getWindowSize?.()?.[0] || 25) - 1;
    ...
    Spinner = class {
      constructor() {
        ...
        this.warning = (message) => {
          return this.stop(
            `${dist_default4.yellow.bold(SYMBOL_WARNING)} ${message}`,
          );
        };
  ...
  "node_modules/@prettier/cli/dist/logger.js"() {
    ...
    Logger2 = class {
      constructor(level, stream) {
        ...
        const transports =
          stream === "stderr" ? [transportToStderr] : [transportToStdout];
        this.level = level;
...
function readFile(filePath, options = DEFAULT_READ_OPTIONS) {
  if (isString7(options)) return readFile(filePath, { encoding: options });
...
async function writeFileAsync(filePath, data, options = DEFAULT_WRITE_OPTIONS) {
  ...
  try {
    ...
    fd = await dist_default36.retry.open(retryOptions)(
      tempPath,
      "w",
    ...
    if (isString7(data)) {
      await dist_default36.retry.write(retryOptions)(
        fd,
        data,
        0,
    ...
    if (
      options.chown &&
      (options.chown.uid !== DEFAULT_USER_UID ||
        options.chown.gid !== DEFAULT_USER_GID)
    ) {
      await dist_default36.attempt.chown(
        tempPath,
        options.chown.uid,
        options.chown.gid,
      );
    ...
    try {
      ...
      await dist_default36.retry.rename(retryOptions)(
        tempPath,
        temp_default.truncate(filePath),
      );
    }
    tempDisposer();
    tempPath = null;
...
function makeCached(options, cache3, prettier) {
  return {
    ...
    async checkWithPath(
      ...
      if (isBoolean2(data?.formatted)) return data.formatted;
      const fileContent =
    ...
    WorkerShim = class extends EventTarget {
      ...
      constructor(url3, options) {
        ...
        if (!url3.startsWith(prefix))
          throw new Error(
            `Only urls that start with "${prefix}" are supported`,
          );
        ...
        __classPrivateFieldSet2(
          this,
          _WorkerShim_worker,
          new Worker(new URL(code)),
          "f",
        );
        __classPrivateFieldGet2(this, _WorkerShim_worker, "f").on(
          "message",
          (data) => {
      ...
      postMessage(message, transfer) {
        const event = new Event("message");
        event["data"] = message;
        __classPrivateFieldGet2(this, _WorkerShim_worker, "f").postMessage(
          event,
          transfer,
        );
      }
    ...
    Worker2 = class {
      ...
      constructor(name, bootloader) {
        this.onClose = (code) => {
          ...
          this.worker.terminate();
          this.reject(
            new error_default(this.name, `Exited with exit code ${code}`),
          );
        };
        this.onError = (error) => {
          if (this.terminated) return;
...
${ready}`;
        ...
        this.tick = () => {
          ...
          if (task.signal) {
            task.signal.addEventListener("abort", worker.terminate, {
              once: true,
            });
          }
          let timeoutId;
        ...
        this.bootloader = this.getWorkerBootloader(
          this.env,
          options.worker.methods,
        );
...
async function runStdin(options, pluginsDefaultOptions, pluginsCustomOptions) {
  ...
  try {
    ...
    process18.exitCode =
      (options.check || options.list) && formatted !== fileContent ? 1 : 0;
  } catch (error) {
...
async function runGlobs(options, pluginsDefaultOptions, pluginsCustomOptions) {
  ...
  const filesExtraPaths = await getFoldersChildrenPaths([
    rootPath,
    ...foldersExtraPaths,
  ]);
  const filesExtraNames = filesExtraPaths.map((filePath) =>
  ...
  const prettierConfigNames = options.config
    ? without2(Object.keys(File2Loader), ["default"]).filter(
        known_default.hasFileName,
      )
    : [];
  const fileNames2parentPaths = (names) =>
    names.flatMap((name) => filesNamesToPaths[name]?.map(path17.dirname) || []);
  const editorConfigPaths = uniq4([
    ...fileNames2parentPaths(editorConfigNames),
    rootPath,
    ...foldersExtraPaths,
  ]);
  const ignorePaths = uniq4([
    ...fileNames2parentPaths(ignoreNames),
    rootPath,
    ...foldersExtraPaths,
  ]);
  const prettierConfigPaths = uniq4([
  ...
  const ignoreManualFilesPaths = ignoreManualFilesNames.map((fileName) =>
    path17.resolve(fileName),
  );
  const ignoreManualFilesContents = await Promise.all(
    ignoreManualFilesPaths.map((filePath) =>
      fs13.readFile(filePath, "utf8").catch(() => ""),
    ),
  );
  ...
  const prettierManualConfigs = await Promise.all(
    prettierManualFilesPaths.map(Loaders.auto),
  );
  ...
  const cacheVersion = dist_default24({
    prettierVersion,
    cliVersion,
    pluginsNames,
    pluginsVersions,
  ...
  stdout.prefixed.debug(() =>
    pathsUnknown
      .map((filePath) =>
        normalizePathSeparatorsToPosix(fastRelativePath(rootPath, filePath)),
      )
      .join("\n"),
  );
...
var makePluggableBin = async () => {
  ...
  for (let i = 0, l = pluginsNames.length; i < l; i++) {
    ...
    for (const option in plugin.options) {
      ...
      if (type2 === "int") {
        const descriptionDefault = isNumber2(initial)
          ? `Defaults to "${initial}"`
          : "";
...
${descriptionDefault}`.trim();
        ...
        bin2 = bin2.option(
          `--${to_kebab_case_default(option)} ${args2}`,
          description,
          { deprecated, section, type: type3, validate },
        );
      } else if (type2 === "boolean") {
        const descriptionDefault = initial
          ? 'Defaults to "true"'
          : 'Defaults to "false"';
        const description = `${descriptionInfo}
${descriptionDefault}`.trim();
...
${descriptionDefault}`.trim();
        ...
        bin2 = bin2.option(
          `--${to_kebab_case_default(option)} ${args2}`,
          description,
          { deprecated, section, enum: values },
        );
      }
    }
    if (plugin.parsers) {
...
var makeWarnedPluggableBin = async () => {
  ...
  if (isString4(args["config"]) || args["config"] === true) {
    exit_default(
      'The "--config" option has been renamed to "--config-path" instead',
    );
  }
