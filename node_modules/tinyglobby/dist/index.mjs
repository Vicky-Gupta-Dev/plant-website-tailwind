...
const escapePosixPath = (path$1) =>
  path$1.replace(POSIX_UNESCAPED_GLOB_SYMBOLS, "\\$&");
const escapeWin32Path = (path$1) =>
  path$1.replace(WIN32_UNESCAPED_GLOB_SYMBOLS, "\\$&");
/**
...
function normalizePattern(pattern, expandDirectories, cwd, props, isIgnore) {
  ...
  if (!isIgnore && props.depthOffset >= 0) {
    ...
    props.root =
      newCommonPath.length > 0 ? posix.join(cwd, ...newCommonPath) : cwd;
  }
  return result;
}
...
function getCrawler(patterns, inputOptions = {}) {
  ...
  const formatExclude = options.absolute
    ? format
    : buildFormat(cwd, props.root, true);
  const fdirOptions = {
    filters: [
      options.debug
        ? (p, isDirectory) => {
  ...
  const relative =
    cwd !== root && !options.absolute && buildRelative(cwd, props.root);
...
async function glob(patternsOrOptions, options) {
  ...
  const isModern =
    isReadonlyArray(patternsOrOptions) || typeof patternsOrOptions === "string";
