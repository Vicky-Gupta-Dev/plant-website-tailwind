...
When the `imports` transform is _not_ specified (i.e. when targeting ESM), the
...
- **production**: If `true`, use production version of functions and don't include debugging
  information. When using React in production mode with the automatic transform, this _must_ be
