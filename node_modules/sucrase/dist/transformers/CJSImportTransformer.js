...
var _getImportExportSpecifierInfo2 = _interopRequireDefault(
  _getImportExportSpecifierInfo,
);
...
var _shouldElideDefaultExport2 = _interopRequireDefault(
  _shouldElideDefaultExport,
);
...
class CJSImportTransformer extends _Transformer2.default {
  ...
  process() {
    ...
    if (
      this.tokens.matches3(
        _types.TokenType._import,
        _types.TokenType.name,
        _types.TokenType.eq,
      )
    ) {
    ...
    if (
      this.tokens.matches1(_types.TokenType._export) &&
    ...
    if (
      this.tokens.matches2(_types.TokenType.name, _types.TokenType.postIncDec)
    ) {
    ...
    if (
      ...
      this.tokens.matches1(_types.TokenType.jsxName)
    ) {
  ...
  processImport() {
    if (
      this.tokens.matches2(_types.TokenType._import, _types.TokenType.parenL)
    ) {
    ...
    _removeMaybeImportAttributes.removeMaybeImportAttributes.call(
      void 0,
      this.tokens,
    );
  ...
  removeImportAndDetectIfShouldElide() {
    ...
    if (
      ...
      !this.tokens.matches1AtIndex(
        this.tokens.currentIndex() + 1,
        _types.TokenType.comma,
      ) &&
      !this.tokens.matchesContextualAtIndex(
        this.tokens.currentIndex() + 1,
        _keywords.ContextualKeyword._from,
      )
    ) {
    ...
    if (
      this.tokens.matches1(_types.TokenType.name) ||
      this.tokens.matches1(_types.TokenType.star)
    ) {
  ...
  processIdentifier() {
    ...
    while (
      ...
      this.tokens.tokens[possibleOpenParenIndex].type ===
        _types.TokenType.parenR
    ) {
  ...
  processExport() {
    if (
      ...
      this.tokens.matches3(
        _types.TokenType._export,
        _types.TokenType._const,
        _types.TokenType._enum,
      )
    ) {
    ...
    if (
      this.tokens.matches2(_types.TokenType._export, _types.TokenType._default)
    ) {
      if (
        this.tokens.matches3(
          _types.TokenType._export,
          _types.TokenType._default,
          _types.TokenType._enum,
        )
      ) {
      ...
      this.tokens.matches2(_types.TokenType._export, _types.TokenType.braceL)
    ) {
      ...
      this.tokens.matchesContextualAtIndex(
        this.tokens.currentIndex() + 1,
        _keywords.ContextualKeyword._type,
      )
    ) {
      // export type {a};
      // export type {a as b};
    ...
    if (
      ...
      this.tokens.matches2(_types.TokenType._export, _types.TokenType.star)
    ) {
  ...
  processAssignment() {
    ...
    if (
      identifierToken.isType ||
      identifierToken.type !== _types.TokenType.name
    ) {
      return false;
    }
  ...
  processPostIncDec() {
    ...
    if (
      index >= 1 &&
      this.tokens.matches1AtIndex(index - 1, _types.TokenType.dot)
    ) {
      return false;
    }
    const identifierName = this.tokens.identifierNameForToken(identifierToken);
  ...
  processExportDefault() {
    ...
    if (
      ...
      _shouldElideDefaultExport2.default.call(
        void 0,
  ...
  processComplexExportVar() {
    ...
    while (true) {
      if (
        ...
        this.tokens.matches1(_types.TokenType.bracketR)
      ) {
        depth--;
        this.tokens.copyToken();
      } else if (
  ...
  processNamedFunction() {
    if (this.tokens.matches1(_types.TokenType._function)) {
      ...
      this.tokens.matches2(_types.TokenType.name, _types.TokenType._function)
    ) {
  ...
  processExportBindings() {
    ...
    while (true) {
      ...
      const specifierInfo = _getImportExportSpecifierInfo2.default.call(
        void 0,
        this.tokens,
      );
      ...
      if (
        this.tokens.matches2(_types.TokenType.comma, _types.TokenType.braceR)
      ) {
        this.tokens.removeToken();
        this.tokens.removeToken();
        break;
