...
function getClassInfo(
  ...
  tokens.nextToken();
  while (
    !tokens.matchesContextIdAndLabel(_types.TokenType.braceR, classContextId)
  ) {
    if (
      ...
      while (isAccessModifier(tokens.currentToken())) {
        ...
        if (
          tokens.matches1(_types.TokenType._declare) ||
          tokens.matches1(_types.TokenType._abstract)
        ) {
      ...
      if (
        tokens.matches1(_types.TokenType.lessThan) ||
        tokens.matches1(_types.TokenType.parenL)
      ) {
...
function processClassHeader(tokens) {
  ...
  while (!tokens.matchesContextIdAndLabel(_types.TokenType.braceL, contextId)) {
    ...
    if (
      tokens.matches1(_types.TokenType._extends) &&
      !tokens.currentToken().isType
    ) {
      hasSuperclass = true;
    }
...
function processConstructor(tokens) {
  ...
  while (
    ...
    if (
      !foundSuperCall &&
      tokens.matches2(_types.TokenType._super, _types.TokenType.parenL)
    ) {
      tokens.nextToken();
