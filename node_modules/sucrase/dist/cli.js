...
function run() {
  _commander2.default
    ...
    .option(
      ...
      "File extension to use for all output files.",
      "js",
    )
    .option(
      "--exclude-dirs <paths>",
      "Names of directories that should not be traversed.",
    )
    .option("-q, --quiet", "Don't print the names of converted files.")
    .option(
      "-t, --transforms <transforms>",
      "Comma-separated list of transforms to run.",
    )
    ...
    .option(
      "--keep-unused-imports",
      "Disable automatic removal of type-only imports/exports.",
    )
    .option(
      "--preserve-dynamic-import",
      "Don't transpile dynamic import() to require.",
    )
    .option(
    ...
    .option(
      "--enable-legacy-babel5-module-interop",
      "Use Babel 5 ESM/CJS interop strategy.",
    )
    .parse(process.argv);
  ...
  const options = {
    ...
    excludeDirs: _commander2.default.excludeDirs
      ? _commander2.default.excludeDirs.split(",")
      : [],
    quiet: _commander2.default.quiet,
    sucraseOptions: {
      transforms: _commander2.default.transforms
        ? _commander2.default.transforms.split(",")
        : [],
      ...
      jsxFragmentPragma:
        _commander2.default.jsxFragmentPragma || "React.Fragment",
      ...
      injectCreateRequireForImportRequire:
        _commander2.default.injectCreateRequireForImportRequire,
      enableLegacyTypeScriptModuleInterop:
        _commander2.default.enableLegacyTypescriptModuleInterop,
      enableLegacyBabel5ModuleInterop:
        _commander2.default.enableLegacyBabel5ModuleInterop,
    },
  };
...
async function runGlob(options) {
  const tsConfigPath = _path.join.call(
    void 0,
    options.project,
    "tsconfig.json",
  );
  ...
  if (include) {
    for (const pattern of include) {
      const globFiles = await _tinyglobby.glob.call(
        void 0,
        _path.join.call(void 0, absProject, pattern),
...
async function updateOptionsFromProject(options) {
  ...
  if (compilerOpts.outDir) {
    options.outDirPath = _path.join.call(
      void 0,
      process.cwd(),
      options.project,
      compilerOpts.outDir,
...
async function buildFile(srcPath, outPath, options) {
  ...
  const transformedCode = _index.transform.call(void 0, code, {
