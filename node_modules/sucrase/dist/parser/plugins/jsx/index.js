...
function jsxReadToken() {
  ...
  while (true) {
    ...
    if (
      ch === _charcodes.charCodes.lessThan ||
      ch === _charcodes.charCodes.leftCurlyBrace
    ) {
    ...
    if (ch === _charcodes.charCodes.lineFeed) {
      ...
      ch !== _charcodes.charCodes.space &&
      ch !== _charcodes.charCodes.carriageReturn &&
      ch !== _charcodes.charCodes.tab
    ) {
...
function jsxReadWord() {
  ...
  do {
    ...
    _identifier.IS_IDENTIFIER_CHAR[ch] ||
    ch === _charcodes.charCodes.dash
  );
...
function jsxParseNamespacedName(identifierRole) {
  ...
  if (!_index.eat.call(void 0, _types.TokenType.colon)) {
    ...
    _base.state.tokens[_base.state.tokens.length - 1].identifierRole =
      identifierRole;
    return;
  }
...
function jsxParseElementName() {
  ...
  if (!hadDot) {
    ...
    if (
      firstChar >= _charcodes.charCodes.lowercaseA &&
      firstChar <= _charcodes.charCodes.lowercaseZ
    ) {
...
function jsxParseOpeningElement(initialTokenIndex) {
  ...
  while (
    !_index.match.call(void 0, _types.TokenType.slash) &&
    !_index.match.call(void 0, _types.TokenType.jsxTagEnd) &&
    !_base.state.error
  ) {
    ...
    if (
      ...
      _base.input.charCodeAt(_base.state.start) ===
        _charcodes.charCodes.lowercaseK &&
      _base.input.charCodeAt(_base.state.start + 1) ===
        _charcodes.charCodes.lowercaseE &&
      _base.input.charCodeAt(_base.state.start + 2) ===
        _charcodes.charCodes.lowercaseY
    ) {
      _base.state.tokens[initialTokenIndex].jsxRole =
        _index.JSXRole.KeyAfterPropSpread;
    }
          ...
          if (_index.match.call(void 0, _types.TokenType.slash)) {
            ...
            if (
              ...
              if (numExplicitChildren === 1) {
                _base.state.tokens[initialTokenIndex].jsxRole =
                  _index.JSXRole.OneChild;
              } else if (numExplicitChildren > 1) {
                _base.state.tokens[initialTokenIndex].jsxRole =
                  _index.JSXRole.StaticChildren;
              }
            }
            return;
          }
...
function nextJSXTagToken() {
  ...
  if (_identifier.IS_IDENTIFIER_START[code]) {
    ...
    code === _charcodes.charCodes.quotationMark ||
    code === _charcodes.charCodes.apostrophe
  ) {
    jsxReadString(code);
  } else {
