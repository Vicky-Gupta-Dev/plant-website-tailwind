    ...
    case tt.parenL:
      ...
      if (isGroupedType) {
        ...
        if (
          ...
          !(
            match(tt.comma) ||
            (match(tt.parenR) && lookaheadType() === tt.arrow)
          )
        ) {
          expect(tt.parenR);
          return;
...
function flowParsePostfixType() {
  ...
  while (
    !canInsertSemicolon() &&
    (match(tt.bracketL) || match(tt.questionDot))
  ) {
    eat(tt.questionDot);
    expect(tt.bracketL);
    if (eat(tt.bracketR)) {
...
export function flowShouldParseExportStar() {
  return (
    match(tt.star) ||
    (isContextual(ContextualKeyword._type) && lookaheadType() === tt.star)
  );
}
...
export function flowParseImportSpecifier() {
  ...
  if (
    ...
    if (
      isTypeKeyword &&
      (match(tt.name) || !!(state.type & TokenType.IS_KEYWORD))
    ) {
      // `import {type foo`
      parseIdentifier();
    }
