...
function tsParseInferType() {
  ...
  if (match(tt._extends)) {
    ...
    if (
      state.error ||
      (!state.inDisallowConditionalTypesContext && match(tt.question))
    ) {
      state.restoreFromSnapshot(snapshot);
    }
  }
...
function isAbstractConstructorSignature() {
  return (
    isContextual(ContextualKeyword._abstract) && lookaheadType() === tt._new
  );
}
...
function tsIsExternalModuleReference() {
  return (
    isContextual(ContextualKeyword._require) && lookaheadType() === tt.parenL
  );
}
...
export function tsParseSubscript(startTokenIndex, noCalls, stopState) {
  ...
  if (match(tt.lessThan) || match(tt.bitShiftL)) {
    ...
    !noCalls &&
    match(tt.questionDot) &&
    lookaheadType() === tt.lessThan
  ) {
